...
## GUI accuracy: preserve the failure
...
We froze **117 unused screenshots**, excluding every filename and exact image hash in prior MiSO screen manifests. The target was 128; some application/type buckets lacked enough unused images, and the [acquisition record](../../evals/acquisition/v4_fresh_screens_v2.json) reports each shortfall. Applications are reused, so this is not an application-disjoint test. The retained baseline scored **60/117 (51.28%)** on regions and **0/117** actual clicks using the grid-cell centers. The change from the old 39.58% region result is a different test sample, not a demonstrated model improvement.

The [fresh-screen result](attempts/screen-confirmation-v1/summary.json) separately reports region classification and whether the predicted grid-cell center actually lands inside the annotated target. The latter uses point-inside-box scoring. This intentionally weak control makes clear why a high region score would still not establish reliable clicking. It is not an official ScreenSpot-Pro run with a dedicated point predictor. [Publisher task](https://github.com/likaixin2000/ScreenSpot-Pro-GUI-Grounding).
