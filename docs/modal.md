...
uv run --locked --group cloud modal run --profile btoo cloud/modal_pilot.py --run-id my-first-l4-pilot
```
...
Two serial calls share a function limited to one L4 container, each with a 180-second function timeout, 120-second startup timeout, no application retries, and a single-use container. Modal can still retry failed container initialization; the local deadline and app stop bound that case. The local caller cancels a call after 330 seconds. The pilot has no endpoint, schedule, minimum warm pool, or persistent serving deployment. Containers stop when the run ends; checkpoint storage remains.
