...
from mmso.workflow_benchmark import JevAdapter, digest as request_digest
...
def main():
    ...
    adapter = JevAdapter(max_spend_usd=.1, key_file=args.key_file)
    ...
    for i, row in enumerate(rows):
        ...
        if failed:
            ...
            try:
                ...
                result = {'status': 'ok', 'probabilities': [raw['answers']['answer']['probabilities'][chr(65+j)] for j in range(len(row['choices']))],
                    'timings': {'request_ms': raw['latency_seconds'] * 1000}, 'usage': raw['usage'], 'cost_usd': raw['cost_usd']}
